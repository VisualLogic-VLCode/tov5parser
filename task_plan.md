...
**当前检查点：** 第 38/51 例 `款式设计_11036309_吴坤.json`（nid `11036309`）首次转换发现 3 个 data-if 的有效 V4 完整运行时条件在 V5 中丢失分支，另有 4 个动作参数属于 V4 源语法错误。用户已明确要求“修复”，进入 Phase 116；只修复 data-if 通用根因，验证后执行固定发布流程。历史案例继续保留，第 39 例不启动。
...
- [ ] 更新第 38 例报告，只暂存相关文件并提交、推送 tov5parser
